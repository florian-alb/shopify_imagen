...
async function submitOpenAiBatch(args: { images: BatchImage[]; settings: Record<string, unknown>; model: string }) {
  ...
  // passed by URL. Normalize like the realtime path, stage it on R2, then point
  // input_reference at that public URL.
  const lines: string[] = [];
    ...
    lines.push(
      JSON.stringify({
        ...
        body: {
          ...
          input_reference: staged.length > 1 ? staged : staged[0]
        }
      })
    );
  }
...
async function pollOpenAiBatch(batchId: string, settings: Record<string, unknown>): Promise<BatchPollResult> {
  ...
  if (status === "failed" || status === "expired" || status === "cancelled") {
    return { state: "failed", error: `OpenAI batch ${status}.` };
  }
      ...
      for (const image of pending) {
        ...
        if (!result || result.error || !result.bytes) {
          await ctx.runMutation(internal.jobs.failImage, {
            imageId: image._id,
            error: result?.error ?? "No batch result returned for this image."
          });
          continue;
        }
        try {
          ...
          await ctx.runMutation(internal.jobs.completeImage, {
            ...
            costUsd: estimateCostUsd(job.imageModel ?? "", usage)
          });
        } catch (error) {
          await ctx.runMutation(internal.jobs.failImage, {
            imageId: image._id,
