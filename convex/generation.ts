...
async function normalizeReferenceImage(sourceUrl: string) {
  ...
  try {
    ...
    throw new Error(
      `Network error fetching reference image from ${sourceUrl}: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
  if (!response.ok)
    throw new Error(
...
function providerIdsFromResponse(
  response: Response,
  payload?: unknown,
): ProviderIds {
...
async function applyBackgroundPostProcessing(args: {
  ...
  const cutoutPng = await sharp(removed.bytes)
    .rotate()
    .ensureAlpha()
    .png()
    .toBuffer();
...
async function optimizeForStorage(
  ...
  try {
    ...
    console.warn(
      `WebP optimization failed, storing original bytes: ${error instanceof Error ? error.message : String(error)}`,
    );
    return {
      bytes,
...
function referenceUrlsForImage(image: {
  sourceImageUrls?: string[];
...
function isCancellableBatchStatus(
...
async function submitGeminiBatch(args: {
  images: BatchImage[];
  settings: Record<string, unknown>;
  model: string;
}) {
...
async function submitOpenAiBatch(args: {
  images: BatchImage[];
  settings: Record<string, unknown>;
  model: string;
}) {
  ...
  const quality = String(
    args.settings.OPENAI_IMAGE_QUALITY ?? env("OPENAI_IMAGE_QUALITY", "medium"),
  );
  const outputFormat = String(
    args.settings.OPENAI_IMAGE_OUTPUT_FORMAT ??
      env("OPENAI_IMAGE_OUTPUT_FORMAT", "jpeg"),
...
async function consumeFirstInlineResponseArray(
  response: Response,
    ...
    if (job.executionMode === "batch" && job.batchId) {
      ...
      batchStatus =
        provider === "gemini"
          ...
          : await cancelOpenAiBatch(job.batchId);
      await ctx.runMutation(internal.jobs.setBatchStatus, {
        jobId: job._id,
        batchStatus,
      });
